...
const BodyInfoPage: VFC<Props> = ({ pageIndex }) => {
  ...
  return (
    <FormContainer<PlanBodyInfoFormValues> pageIndex={pageIndex} submitFunc={registerBodyInfo} title={'✏️ 目標体重を設定する'} open={open} handleClose={() => setOpen(false)}>
      <InputField required value={getStringTypeDate(new Date(), 'YYYY-MM-DD')} min={getStringTypeDate(new Date(), 'YYYY-MM-DD')} title='日付' type='date' placeholder='60' formConf={{ name: 'date', option: { required: true } }} />
