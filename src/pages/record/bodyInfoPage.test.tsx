...
import { createBodyInfoHistories } from '~/tests/mocks/datum/createBodyInfoHistories';
...
describe('Integration Test', () => {
  const renderPage = testRendererUsingApolloClientMock(<BodyInfoPage pageIndex={0} />, [createBodyInfoHistories]);
  
  it('必須項目が入力されている状態で送信ボタンをクリックした場合、Snackbarが表示される', async () => {
    renderPage();
  ...
  it('必須項目が入力されていない状態で送信ボタンをクリックした場合、登録処理がが実行されず、Snackbarが表示されない', async () => {
    renderPage();
    const submitButton = screen.getByTestId('submit');
