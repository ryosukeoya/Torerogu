...
type Props = {
  todaysTrainings?: GetTrainingTrainingTypeQuery['trainings'];
  setTrainings: SetterOrUpdater<GetTrainingTrainingTypeQuery['trainings'] | undefined>;
};

const HomePage: VFC<Props> = ({ todaysTrainings, setTrainings }) => {
  ...
  return (
    <div css={pageTemplate.contentArea}>
      ...
      {todaysTrainings?.length === 0 && <p role='no-data'>※ 本日予定のトレーニングはありません</p>}
    </div>
  );
};
